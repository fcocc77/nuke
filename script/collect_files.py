...
class collect_files():
    ...
    def collect(self):
        ...
        # Cuenta todos los archivos que se van a copiar, es necesario para calcular el porcentaje para el progreso
        box_file = ['file', 'proxy']
        count_total = 0
    ...
    def fila_path_modify(self, file_path):
        ...
        root_name = '[file dirname [value root.name]]/..'
        ...
        self.sequence_find = sorted(fnmatch.filter(
            os.listdir(self.src_dir), base_name + '*'))

        self.action = 1

        self.dst_box = root_name + '/assets/' + name_folder + '/' + file

        if os.path.normpath(self.project_dir) in os.path.normpath(self.src_dir):

            # crea ruta interna del projecto
            ...
            self.dst_box = root_name + inside_file
            self.action = 0
